...
def main():
    # prepare data
    file_exists = exists("zep.csv")
    use_backupfile = False
    if use_backupfile:
        rows = open_file('zep_backup.csv')
    elif not file_exists:
        crawl_data()
        rows = open_file('zep.csv')
    else:
        rows = open_file('zep.csv')
    # group data into trees
    all_product = store_source(rows)
    print('Welcome to find your best zep cleaing product')
...
def crawl_data():
    '''Funciton that used for crawling data from amzon, the seach_query is set to 'zep'
    and crawl for multiple pages of the website
    '''
    headers = {
...
## store all product in a list and remove duplication
...
def intersection(use=all_product, price=all_product, rating=all_product, count=all_product):
...
def get_use_input(rows):
    print("The typical search key word include: carpet, spray, commercial, glass and so on")
    use = input('what type of cleaning product would you want? ')
...
def get_price_input(rows):
    print("The typical price range is 0 ~ 100")
    try:
...
def get_counts_input(rows):
    print("The typical rating count range is 0 ~ 2000")
    try:
